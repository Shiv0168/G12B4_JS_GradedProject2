  ...
  if (event.keyCode === 13) {
    $(document).ready(function () {
      $.getJSON("Data.json", function (data) {
        ...
        for (i = 0; i <= 5; i++) {
          if (
            data.resume[i].basics.AppliedFor.toLowerCase() ==
...
function displayfilter(data, k) {
  ...
  document.querySelector(
    ".link"
  ).href = `${data.resume[k].basics.profiles.url}`;
...
function back() {
  ...
  if (array[z] != undefined) {
    document.getElementById("next").style.visibility = "visible";
    if (z == 0) document.getElementById("prev").style.visibility = "hidden";
...
function display(datas, k) {
  ...
  document.querySelector(
    ".link"
  ).href = `${datas.resume[k].basics.profiles.url}`;
